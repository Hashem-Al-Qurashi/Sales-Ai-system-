#!/usr/bin/env python3
"""
Framework Vectorization Script

Converts the 17 manually chunked Hormozi frameworks into embeddings 
and stores them in the vector database for RAG retrieval.

Usage:
    python vectorize_frameworks.py
"""
...
class FrameworkVectorizer:
    """Handles vectorization and storage of framework chunks."""
    
    def __init__(self):
        self.embedder = OpenAIEmbedder()
        self.vector_store = ChromaVectorStore()
        
    async def vectorize_and_store_frameworks(self) -> Dict[str, Any]:
        """
        Vectorize all framework chunks and store in vector database.
        
        Returns:
            Dict with processing statistics
        """
        logger.info("Starting framework vectorization process...")
        
        results = {
            "total_chunks": len(FRAMEWORK_CHUNKS),
            "processed": 0,
            "failed": 0,
            "errors": [],
            "start_time": datetime.now().isoformat(),
            "collection_name": "hormozi_frameworks"
        }
        
        try:
            # Initialize vector store collection
            await self.vector_store.create_collection("hormozi_frameworks")
            
            # Process each framework chunk
            for chunk_data in FRAMEWORK_CHUNKS:
                try:
                    logger.info(f"Processing chunk: {chunk_data['chunk_id']}")
                    
                    # Generate embedding
                    embedding = await self.embedder.embed_text(chunk_data["text"])
                    ...
                    metadata = {
                        ...
                        "contains_list": chunk_data["contains_list"],
                        "contains_example": chunk_data["contains_example"],
                        "business_logic_intact": chunk_data["business_logic_intact"],
                        "validation_passed": chunk_data["validation_passed"],
                        "preserves_complete_concept": chunk_data["preserves_complete_concept"],
                    ...
                    results["processed"] += 1
                    logger.info(f"Successfully processed: {chunk_data['chunk_id']}")
                    
                except Exception as e:
                    error_msg = f"Failed to process {chunk_data['chunk_id']}: {str(e)}"
            ...
            results_file = Path(settings.DATA_DIR) / "embeddings" / "framework_vectorization_results.json"
            with open(results_file, "w") as f:
                json.dump(results, f, indent=2)
            
            logger.info(f"Framework vectorization complete: {results['processed']}/{results['total_chunks']} chunks processed")
            
        except Exception as e:
            logger.error(f"Framework vectorization failed: {str(e)}")
...
async def main():
    ...
    print("🚀 Starting Hormozi Framework Vectorization")
    print(f"📊 Processing {len(FRAMEWORK_CHUNKS)} framework chunks...")
    
    vectorizer = FrameworkVectorizer()
    results = await vectorizer.vectorize_and_store_frameworks()
    ...
    print("   - 'How do I stack bonuses effectively?'")
